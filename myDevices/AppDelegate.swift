...
class AppDelegate: UIResponder, UIApplicationDelegate {
        ...
        do {
            if let results = try managedObjectContext.executeFetchRequest(fetchRequest) as? [NSManagedObject] {
                for result in results {
                    if let deviceType = result.valueForKey("deviceType") as? String,
    ...
    func addTestData() {
        guard let entity = NSEntityDescription.entityForName("Device", inManagedObjectContext: managedObjectContext) else {
            fatalError("Could not find entity with this name")
        }
        for i in 1...25 {
            ...
            device.setValue(i % 3 == 0 ? "watch":"phone", forKey: "deviceType")
        }
    }
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "rit.ua.myDevices" in the application's documents Application Support directory.
